...
@section('title', 'Transaksi - Izin Karyawan')
@section('subtitle', 'Transaksi Izin Karyawan')
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-2">
                    <div>
                        <h4 class="mb-0">Izin Karyawan</h4>
                        <p class="text-muted mb-0">
                            Total: <span class="fw-bold">{{ $izins->total() }}</span> izin
                        </p>
                    </div>
                    ...
                    <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2">
                        <form id="filterForm" method="GET" action="{{ url()->current() }}"
                            ...
                            <select id="searchJenis" name="searchJenis" class="form-select form-select-sm" style="min-width: 220px;">
                                @if(request()->filled('searchJenis'))
                                    <option value="{{ request('searchJenis') }}" selected>Selected Jenis Izin</option>
                                @endif
                            </select>

                            {{-- Status (select) biar 1 style sama Cuti --}}
                        ...
                        <a href="{{ route('admin.transaksi.izin-karyawan.create') }}" class="btn btn-primary btn-sm">
            ...
            <div class="card-body">
                @if($izins->count() === 0)
                    <div class="alert alert-light-primary mb-0">
                        <i class="bi bi-info-circle me-1"></i> No izin found.
                    </div>
                @else
                    <div class="table-responsive">
                        <table class="table table-lg align-middle">
                            ...
                            @foreach($izins as $i => $izin)
                                <tr>
                                    <td class="text-muted">{{ $izins->firstItem() + $i }}</td>
                                    <td class="fw-semibold">
                                        {{ $izin->nama_karyawan ?? '-' }}
                                        <div class="text-muted small">ID: <span class="font-monospace">{{ $izin->m_karyawan_id }}</span></div>
                                    </td>
                                    <td>{{ $izin->nama_perusahaan ?? '-' }}</td>
                                    <td>{{ optional($izin->jenisIzin)->nama_izin ?? $izin->m_jenis_izin_id }}</td>
                                    <td class="text-muted small">
                                        {{ $izin->start_date }} → {{ $izin->end_date }}
                                    </td>
                                    <td class="fw-semibold">{{ $izin->durasi ?? '-' }}</td>
                                    <td>
                                        @php($st = $izin->status)
                                        <span class="badge
                                    ...
                                    <td class="text-muted small">{{ optional($izin->created_at)->format('Y-m-d H:i') }}</td>
                                    <td class="text-end">
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li>
                                                    <a class="dropdown-item" href="{{ route('admin.transaksi.izin-karyawan.show', $izin->id) }}">
                                                ...
                                                <li>
                                                    <button type="button" class="dropdown-item text-danger btn-delete"
                                                            data-url="{{ route('admin.transaksi.izin-karyawan.destroy', $izin->id) }}"
                                                            data-name="{{ $izin->nama_karyawan ?? 'this request' }}">
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mt-3">
                        <div class="text-muted small">
                            Showing <b>{{ $izins->firstItem() }}</b> to <b>{{ $izins->lastItem() }}</b>
                            of <b>{{ $izins->total() }}</b> results
                        </div>
                        <div>{{ $izins->links() }}</div>
                    </div>
                @endif
...
@push('scripts')
    <script>
        $(function () {
                ...
                $el.select2({
                    ...
                    ajax: {
                        ...
                        data: function (params) {
                            return { q: params.term || '', page: params.page || 1, perPage: 20 };
                        },
            ...
            initSelect2Ajax($('#searchKaryawan'), @json(route('admin.transaksi.izin-karyawan.karyawan-options')), 'Karyawan...');
            initSelect2Ajax($('#searchCompany'),  @json(route('admin.transaksi.izin-karyawan.company-options')),  'Company...');
            initSelect2Ajax($('#searchJenis'),    @json(route('admin.transaksi.izin-karyawan.jenis-izin-options')), 'Jenis Izin...');
            ...
            $(document).on('click', '.btn-delete', function () {
                ...
                const name = String($(this).data('name') || 'this request');
