...
@section('title', 'Transaksi - Surat Peringatan')
                ...
                <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-2">
                    <div>
                        ...
                        <p class="text-muted mb-0">
                            Total: <span class="fw-bold">{{ $sps->total() }}</span> surat
                        </p>
                    </div>
                    ...
                    <div class="d-flex flex-column flex-md-row align-items-stretch align-items-md-center gap-2">
                        <form id="filterForm" method="GET" action="{{ url()->current() }}"
                            ...
                            <select id="searchJenis" name="searchJenis" class="form-select form-select-sm" style="min-width: 220px;">
                                @if(request()->filled('searchJenis'))
                                    <option value="{{ request('searchJenis') }}" selected>Selected Jenis SP</option>
                                @endif
                            </select>

                            <select id="searchStatus" name="searchStatus" class="form-select form-select-sm" style="min-width: 190px;">
                            ...
                            <div class="input-group input-group-sm" style="min-width: 180px; max-height: 40px;">
                                ...
                                <input id="searchNomor" type="text" class="form-control"
                                       name="searchNomor" placeholder="Nomor..."
                                       value="{{ request('searchNomor') }}" autocomplete="off">
                            </div>
                        ...
                        <a href="{{ route('admin.transaksi.surat-peringatan.create') }}" class="btn btn-primary btn-sm">
            ...
            <div class="card-body">
                @if($sps->count() === 0)
                    <div class="alert alert-light-primary mb-0">
                        <i class="bi bi-info-circle me-1"></i> Tidak ada data surat peringatan.
                    </div>
                @else
                    <div class="table-responsive">
                        <table class="table table-lg align-middle">
                            ...
                            @foreach($sps as $i => $sp)
                                @php($st = $sp->status)
                                <tr>
                                    <td class="text-muted">{{ $sps->firstItem() + $i }}</td>
                                    <td class="fw-semibold">{{ $sp->nomor ?? '-' }}</td>
                                    <td class="fw-semibold">
                                        {{ $sp->nama_karyawan ?? '-' }}
                                        <div class="text-muted small">ID: <span class="font-monospace">{{ $sp->m_karyawan_id }}</span></div>
                                    </td>
                                    <td>{{ $sp->nama_perusahaan ?? '-' }}</td>
                                    <td>{{ $sp->m_jenis_sp_id ?? '-' }}</td>
                                    ...
                                    <td class="text-muted small">{{ optional($sp->created_at)->format('Y-m-d H:i') }}</td>
                                    <td class="text-end">
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li>
                                                    <a class="dropdown-item" href="{{ route('admin.transaksi.surat-peringatan.show', $sp->id) }}">
                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mt-3">
                        <div class="text-muted small">
                            Showing <b>{{ $sps->firstItem() }}</b> to <b>{{ $sps->lastItem() }}</b>
                            of <b>{{ $sps->total() }}</b> results
                        </div>
                        <div>{{ $sps->links() }}</div>
                    </div>
                @endif
...
@push('scripts')
    <script>
        $(function () {
            ...
            initSelect2Ajax($('#searchKaryawan'), @json(route('admin.transaksi.surat-peringatan.karyawan-options')), 'Karyawan...');
            initSelect2Ajax($('#searchCompany'),  @json(route('admin.transaksi.surat-peringatan.company-options')),  'Company...');
            initSelect2Ajax($('#searchJenis'),    @json(route('admin.transaksi.surat-peringatan.jenis-sp-options')), 'Jenis SP...');

            $('#searchStatus').on('change', debouncedSubmit);
            $('#searchNomor').on('input', debouncedSubmit);
